...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map(({ title, card_subtitle, rank_value, type_name, pic, cover, cover_url, url, uri, rating, year, release_date, good_rating_stats, null_rating_reason, tags, description, comment, id }) => {
            let tags_text = '',
                type_name_text = '',
                release_text = '',
                ...
                rank_value_text = '',
                rank_count_text = '';
            let rate = rating ? `${rating.value.toFixed(1)}分` : null_rating_reason;
            if (!rate) {
                rate = '暂无评分';
            }
            // 描述
